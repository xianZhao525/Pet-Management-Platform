...
import com.example.backend.entity.Adoption.AdoptionStatus;
...
public interface AdoptionRepository extends JpaRepository<Adoption, Long> {
        ...
        // 检查用户是否已经申请过某个宠物
        @Query("SELECT COUNT(a) > 0 FROM Adoption a WHERE a.pet.id = :petId AND a.user.id = :userId AND a.status = :status")
        boolean existsByPetIdAndUserIdAndStatus(@Param("petId") Long petId,
                        @Param("userId") Long userId,
                        @Param("status") AdoptionStatus status);

        // 添加一个更方便的方法
        List<Adoption> findByPetIdAndUserId(Long petId, Long userId);
}